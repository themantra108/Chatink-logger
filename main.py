...
def calc_score(r):
    """Calculate Trend Score based on strict technical thresholds"""
    try:
        def f(v): 
...
def normalize_signature(df):
    """Create unique signature for strict deduplication"""
...
async def run_bot():
    ...
    async with async_playwright() as p:
        ...
        # Optimization: Block images/fonts to speed up loading
        ...
        try:
            ...
            # Smart Wait: Wait for actual data rows
    ...
    # Logic: Select the largest table (usually the data table)
    df = max(dfs, key=len).dropna(how='all').astype(str)
    
    # --- 2. CLEAN & TRANSFORM ---
    print("   ⚙️ Transforming Data...")
    
    # Clean Headers (Remove '_Sort_table_by...')
    ...
    if old.empty:
        ...
        # Compare signature (exclude timestamp column)
        ...
        if not compare_cols: 
            ...
            # Filter rows where signature is NOT in old data
    ...
    # Fetch FRESH full history (so dashboard includes what we just added)
    ...
    df_history['_t'] = pd.to_datetime(df_history['Scraped_At'], errors='coerce')
    df_history = df_history.sort_values(['_d', '_t'], ascending=[False, False])
    
    # Dedup for Dashboard (One row per date/entry)
    df_dash = df_history.drop_duplicates(subset=['Full_Date'], keep='first')
    df_dash = df_dash.drop(columns=['_d', '_t', 'Scraped_At'], errors='ignore')
    
    # Recalculate score for consistent coloring
    ...
    # Clean Setup (Delete/Recreate) - Safest way to clear formatting
    try: sh_dash.del_worksheet(sh_dash.sheet1)
    except: pass
    ws_dash = sh_dash.add_worksheet(title="Sheet1", rows=1000, cols=20)
    
    # Upload Data (Hide the helper __Score__ column from text)
    upload_df = df_dash.drop(columns=['__Score__'])
    set_with_dataframe(ws_dash, upload_df, include_column_header=True)

    # --- 6. APPLY TRAFFIC LIGHT FORMATTING ---
    print("      -> Applying Visual Rules...")
    
    idx = {c: upload_df.columns.get_loc(c) for c in upload_df.columns}
    reqs = []
    
    # A. Number Format (0.00)
    reqs.append({
        "repeatCell": {
    ...
    # C. Sidebar Trend Logic (Coloring Column A based on Score)
    scores = df_dash['__Score__'].iloc[::-1] # Process Oldest to Newest
    ...
    # Apply Sidebar Colors (Reverse back to Newest First)
    ...
    # Header Styling
    h_bg = {"red":0,"green":0.8,"blue":0} if trend == 1 else ({"red":1,"green":0,"blue":0} if trend == -1 else {"red":1,"green":1,"blue":1})
    h_txt = {"red":1,"green":1,"blue":1} if trend != 0 else {"red":0,"green":0,"blue":0}
    reqs.append({"updateCells": {"range": {"sheetId": ws_dash.id, "startRowIndex": 0, "endRowIndex": 1,
                     "startColumnIndex": 0, "endColumnIndex": 1}, "rows": [{"values": [{"userEnteredValue": {"stringValue": upload_df.columns[0]},
                     "userEnteredFormat": {"backgroundColor": h_bg, "textFormat": {"foregroundColor": h_txt, "bold": True}, "horizontalAlignment": "CENTER"}}]}],
                     "fields": "userEnteredValue,userEnteredFormat"}})

    # Batch execute formatting
    if reqs:
        try: ws_dash.spreadsheet.batch_update({"requests": reqs})
        except Exception as e: print(f"Format Warning: {e}")
